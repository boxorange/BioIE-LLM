...
class FalconPrompter:
    ...
    def get_kegg_prompt(self):
        ...
        entity_q = lambda entity: ">>QUESTION<< Which genes are associated with \"{x}\"?\n>>ANSWER<<".format(x=entity)
        entity_a = lambda entities: " {x}\n".format(x=entities)
        
        #entity_q = lambda entity: "Which genes are associated with \"{x}\"?".format(x=entity)
        #entity_a = lambda entities: " {x}\n".format(x=entities)
